...
<!--
  ...
  enforce it. This Claude master copy is the same constitution as `copilot-instructions.md`; it
  differs only in its deployment target (`/CLAUDE.md`) and in composing additional private
  operational manuals through Claude's `@import` mechanism. The full constitution and those manuals
  are maintained in the project's private repository.
-->
...
One constitution, two deployment targets: this Claude copy and the Copilot copy carry the same
authority and differ only in where each is deployed and the agent-specific context each composes. Keeping
them in lockstep is itself a governed rule — the two agents must never operate under divergent law.
...
project's AI agents (this `CLAUDE.md` and `copilot-instructions.md`), each deployed to its agent's
instruction path._
